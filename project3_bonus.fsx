...
let count = fsi.CommandLineArgs.[3] |> int
...
let mutable failId = 0

while failCounter < count do
    failId <- rnd.Next(0,numNodes - 1)
    while List.contains failId failedList do
        failId <- rnd.Next(0,numNodes - 1)
    failedList <- List.append failedList [ failId ]
    ...
    let rec loop() =
        actor {
            ...
            | RevertPredecessor(nodeRef, initialList) -> 
                mailbox.Sender() <! StabilizeReceiver(predecessor, initialList)
    ...
    let rec loop() =
        actor {
            ...
            | LookupDone(_) ->
                ...
                hops <- 0.0
                if lookups = (numNodes*numRequests |> float) - (numRequests*failedList.Length |> float) then 
