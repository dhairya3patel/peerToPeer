    ...
    let rec loop () =
        actor {
            ...
            | FindSuccessor (nodeRef, initialList) ->
                ...
                if
                    not (List.isEmpty fingerTable)
                    && not (isNull successor)
                then
                    if numId > selfAddress && numId < succId then
                        ...
                        if fingerTable.Length = m then
                            while not tempBreak && i >= 0 do
                                fingerId <-
                                    (fst(fingerTable.[i]).Path.Name.Split('_').[1])
                                    |> int
                            ...
                            if
                                tempBreak
                                && not (List.contains (nodeRef.Path.Name.Split('_').[1] |> int) failedList)
                            then
                                fst (fingerTable.[i])
                                <! FindSuccessor(nodeRef, initialList)
                            else
            ...
            | StaticInitiate (initialList) ->
                if not (List.contains (mailbox.Self.Path.Name.Split('_').[1] |> int) failedList)
                    && not ( List.contains
                            (fst(fingerTable.[0]).Path.Name.Split("_").[1]
                             |> int)
                            failedList
                    )
                then
            ...
            | SetPredecessor (nodeRef) -> predecessor <- nodeRef
    ...
    let rec loop () =
        actor {
            ...
            | Start (_) ->
                ...
                m <-
                    Math.Ceiling(Math.Log(initialList.Length - failedList.Length |> float, 2.))
                    |> int
            ...
            | LookupDone (_) ->
                ...
                lookups <- lookups + 1.0
                Console.WriteLine(lookups.ToString() + " " + hops.ToString() + " " + keyList.Length.ToString())
                hops <- 0.0
